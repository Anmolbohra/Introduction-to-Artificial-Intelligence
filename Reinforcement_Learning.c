...
class RLAgent:
    ...
    def epsGreedy(self,eps :float=0.1) -> action_t:
        ...
        if(sample < eps):
            ...
            action = self.idxAction[action]
        else:
            action = self.bestAction(self.state)
        return action
        # return self.idxAction[action]
    ...
    def qstep(self):
        # pass
        #<<< YOUR CODE HERE >>>      
        self.action = self.epsGreedy()
        nextState, reward = self.interaction.takeAction(self.action)
        bestNextVal = np.max(self.Q[nextState,:])
        delta = (self.gamma*bestNextVal + reward) - self.Q[self.state,self.actionIdx[self.action]]
        self.Q[self.state,self.actionIdx[self.action]] += self.alpha*delta
